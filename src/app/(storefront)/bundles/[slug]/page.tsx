...
export async function generateMetadata({
  ...
  return {
    title: bundle
      ? t("metaTitle", { name: localizedField(locale, bundle.nameEn, bundle.nameAr), brand: tCommon("brand") })
  ...
  return (
    <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
      <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-surface-secondary">
        <Image src={bundle.mainImageUrl} alt={bundleName} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" priority />
      </div>
      <div className="flex flex-col gap-4">
        <h1 className="font-heading text-3xl font-semibold text-ink">{bundleName}</h1>
        ...
        <p className="text-ink-muted">{bundleDescription}</p>

        <div className="rounded-2xl border border-border p-4">
          <p className="mb-3 text-sm font-medium text-ink">{t("whatsIncluded")}</p>
          <div className="flex flex-col gap-3">
            {bundle.items.map((item) => {
              const itemName = localizedField(locale, item.product.nameEn, item.product.nameAr);
              return (
                <Link
                  key={item.id}
                ...
                >
                  <div className="relative h-12 w-12 overflow-hidden rounded-lg bg-surface-secondary">
                    <Image src={item.product.mainImageUrl} alt={itemName} fill sizes="48px" className="object-cover" />
                  </div>
